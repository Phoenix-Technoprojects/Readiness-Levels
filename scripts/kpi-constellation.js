  ...
  .then(data => {
    ...
    for (let i = 0; i < nodeList.length; i++) {
      for (let j = i + 1; j < nodeList.length; j++) {
        if (nodeList[i].group !== nodeList[j].group && Math.random() < 0.25) {
          links.push({
    ...
    const dynamicCharge = () =>
      forceManyBody().strength(d => -90 - 15 * Math.sin(Date.now() / 4000 + d.index));
    ...
    function ticked() {
      ...
      nodes.forEach(d => {
        ...
        if (currentMode === "storm") {
          chaosFactor = 0.012 + 0.005 * Math.sin(tickCount / 20);
        }
